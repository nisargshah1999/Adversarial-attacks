...
def pgd_attack(data, target, model, epsilon, num_steps, alpha=0.2):    
    ...
    for _ in range(num_steps):
        ...
        loss.backward()
        # Collect datagrad
        data_grad = perturbed_data.grad.sign()

        # Create the perturbed image by adjusting each pixel of the input image
        perturbed_data = perturbed_data.detach() + epsilon*data_grad

        # clipping to maintain in the range
...
for eps in epsilons[:]:
    acc, ex = test_pgd(net, device, testloader, eps,num_steps = 2)
    accuracies.append(acc)
    examples.append(ex)

try:
    os.path.mkdir(os.path.join(args.output_path,'results_pgd_linf'))
except  FileExistsError:
    pass
...
for index, eps in enumerate(epsilons):
    for batch in examples[index]:      
        for i in range(len(batch[0])):
            ...
            plt.imsave(f'{args.output_path}/results_pgd_linf/{eps}_{init_pred}_{final_pred}.png',adv_ex)